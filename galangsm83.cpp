...
#define sm83reg8(x) insert(new GAParameterSM83Reg8((x)))
//16-bit register for most instructions.
#define sm83reg16(x) insert(new GAParameterSM83Reg16((x),false))
//16-bit register for push and pop, with AF in place of SP.
#define sm83reg16stk(x) insert(new GAParameterSM83Reg16((x),true))
#define sm83cond(x) insert(new GAParameterSM83Cond((x)))
#define sm83rsthandler(x) insert(new GAParameterSM83ResetHandler((x)))
...
GALangSM83::GALangSM83() {
    ...
    insert(mnem("rst", 1, "\xc7", "\xc7"))
        ...
        ->sm83rsthandler("\x38")  // This should *not* be shifted like a regular parameter.
        ;
...
/* SM83 has rather unique register names.  We define support here,
 * and use the preprocessor so that they needn't be added to the
 * GAParameterGroup class. */
GAParameterSM83Reg8::GAParameterSM83Reg8(const char* mask){
    setMask(mask);
}
int GAParameterSM83Reg8::match(GAParserOperand *op, int len){
    ...
    switch(c){
        //FIXME: The SM83 Register Order might be B, C, D, E, H, L, [HL], A
    case 'b':
    case 'c':
    case 'd':
    case 'e':
...
QString GAParameterSM83Reg8::decode(GALanguage *lang,
...
void GAParameterSM83Reg8::encode(GALanguage *lang,
...
GAParameterSM83Reg16::GAParameterSM83Reg16(const char* mask, bool stk){
    setMask(mask);
    if(stk)
        regnames[3]="af";
}
int GAParameterSM83Reg16::match(GAParserOperand *op, int len){
    if(op->prefix!=this->prefix)
        return 0; //Wrong type.
...
QString GAParameterSM83Reg16::decode(GALanguage *lang,
...
void GAParameterSM83Reg16::encode(GALanguage *lang,
                                 uint64_t adr, QByteArray &bytes,
                                 GAParserOperand op,
                                 int inslen){
    uint64_t val;
...
GAParameterSM83Cond::GAParameterSM83Cond(const char* mask){
    setMask(mask);
}
int GAParameterSM83Cond::match(GAParserOperand *op, int len){
    if(op->prefix!=this->prefix)
        return 0; //Wrong type.
...
QString GAParameterSM83Cond::decode(GALanguage *lang,
                                     uint64_t adr, const char *bytes,
                                    int inslen){
    uint64_t p=rawdecode(lang,adr,bytes,inslen);
    assert(p<4); //Reg count on this architecture.
...
void GAParameterSM83Cond::encode(GALanguage *lang,
                                  uint64_t adr, QByteArray &bytes,
                                  GAParserOperand op,
                                  int inslen){
    uint64_t val;
...
GAParameterSM83ResetHandler::GAParameterSM83ResetHandler(const char* mask){
    setMask(mask);
}
int GAParameterSM83ResetHandler::match(GAParserOperand *op, int len){
...
QString GAParameterSM83ResetHandler::decode(GALanguage *lang, uint64_t adr, const char *bytes, int inslen){
    uint64_t p=rawdecode(lang,adr,bytes,inslen)
                 <<3;   //Shift to expand it.
    return QString::asprintf("0x%02x",(unsigned int) p);
}
void GAParameterSM83ResetHandler::encode(GALanguage *lang,
            uint64_t adr, QByteArray &bytes,
            GAParserOperand op,
            int inslen
            ){
